...
class RealXarm7ArmOnlyEnvBase(RealEnvBase):
    action_space = Box(
        low=np.array(
            [
                -2 * np.pi,
        ...
        **kwargs,
        ...
        self.body_config_list = [
            ArmConfig(
                ...
                gripper_joint_idxes=np.array([], dtype=np.int64),
                gripper_joint_idxes_in_gripper_joint_pos=np.array([], dtype=np.int64),
                eef_idx=0,
                init_arm_joint_pos=self.init_qpos[0:7],
                init_gripper_joint_pos=np.array([], dtype=np.float64),
            )
        ]
        ...
        self.xarm_api.set_collision_sensitivity(4)
        time.sleep(0.2)
