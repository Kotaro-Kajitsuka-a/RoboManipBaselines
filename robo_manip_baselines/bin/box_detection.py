...
BOX_DEPTH_M = 0.1140
# base_center -> camera の外部パラメータ
BASE_CENTER_T_PATH = "robo_manip_baselines/calib/base_center_T.calib"
# RealSense 設定
...
# ==== RealSense 初期化 ========================================================
base_T_cam = np.loadtxt(BASE_CENTER_T_PATH).astype(np.float32)
    ...
    while True:
        ...
        if ids is not None and len(ids) > 0:
            ...
            if retval > 0:
                ...
                cam_T_box[:3, 3] = box_center_cam.astype(np.float32)
                base_T_box = base_T_cam @ cam_T_box
                box_center_base = base_T_box[:3, 3]
                rpy_base_box = rotmat_to_rpy_deg(base_T_box[:3, :3])
        ...
        if board_pose_ok:
            ...
            if box_pose_ok and box_center_base is not None:
                line4 = (f"Box center (base): "
                         f"({box_center_base[0]:.3f}, {box_center_base[1]:.3f}, {box_center_base[2]:.3f}) m")
                cv2.putText(img, line4, (12, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55,
                            (180, 200, 255), 2, cv2.LINE_AA)
                y += 24
                line5 = (f"Box RPY (base): "
                         f"({rpy_base_box[0]:.1f}, {rpy_base_box[1]:.1f}, {rpy_base_box[2]:.1f}) deg")
                cv2.putText(img, line5, (12, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55,
