...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create main_eval.hdf5 for all episodes in a dataset directory."
    )
    parser.add_argument("dataset_dir", type=str, help="Path to dataset directory")
    parser.add_argument("pt_path", type=str, help="Path to YOLO pose .pt file")
...
def _select_keypoints_xy(result) -> np.ndarray:
    ...
    if result.boxes is not None and result.boxes.conf is not None and len(result.boxes.conf) > 0:
        det_idx = int(result.boxes.conf.detach().cpu().numpy().argmax())
    else:
        det_idx = 0

    pts = keypoints.xy[det_idx].detach().cpu().numpy()
    if pts.shape[0] < 4:
        raise RuntimeError(f"Detected keypoints are insufficient: {pts.shape}")
    return pts


def _solve_pnp_from_keypoints_local(
    keypoints_xy: np.ndarray, K: np.ndarray, dist_coeffs: np.ndarray
):
...
def estimate_box_pose_sequences(
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "ultralytics is required. install it in .venv, e.g. `pip install ultralytics`."
        ) from exc
    ...
    result_video_path = video_path.with_name("front_rgb_image_estimation_result.rmb.mp4")
    ...
    def run_single_pass(independent_mode: bool):
        cap = cv2.VideoCapture(str(video_path))
        if not cap.isOpened():
            raise RuntimeError(f"Failed to open video: {video_path}")
        ...
        try:
            for i in range(num_steps):
                ...
                try:
                    if result is None:
                        raise RuntimeError("No YOLO result.")
                    pts = _select_keypoints_xy(result)
                    pnp = _solve_pnp_from_keypoints_local(pts, K, dist_coeffs)
                    if pnp is None:
                    ...
                    if elapsed_seconds <= T_MAX_SECONDS:
                        ...
                        if WARNING_CONSECUTIVE_MISSING <= consecutive_missing <= MAX_CONSECUTIVE_MISSING:
                            mode_name = "independent" if independent_mode else "tracking"
                            print(
                                "\033[1;31m"
                                f"[CreateBoxPoseEvalHdf5][SEVERE] {consecutive_missing} consecutive failures "
                                f"in {mode_name} mode at frame {i} ({elapsed_seconds:.3f}s). "
                        ...
                        if consecutive_missing > MAX_CONSECUTIVE_MISSING:
                            if failure_message is None:
                                mode_name = "independent" if independent_mode else "tracking"
                                failure_message = (
                                    f"Keypoints missing for {consecutive_missing} consecutive frames "
                                    f"in {mode_name} mode at frame {i} ({elapsed_seconds:.3f}s). "
    ...
    try:
        return run_single_pass(independent_mode=False)
    except RuntimeError as exc:
        if "consecutive frames in tracking mode" not in str(exc):
            raise
        print(
            f"[CreateBoxPoseEvalHdf5] More than {MAX_CONSECUTIVE_MISSING} consecutive failures in tracking mode. "
            "Retrying whole video with independent-frame prediction."
        )

    return run_single_pass(independent_mode=True)
...
def process_single_episode(hdf5_path: Path, pt_path: Path) -> None:
...
def main() -> None:
    ...
    for idx, hdf5_path in enumerate(hdf5_paths, start=1):
        ...
        process_single_episode(hdf5_path, pt_path)
