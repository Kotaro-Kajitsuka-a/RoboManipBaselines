...
def resolve_recorded_path(value) -> Path:
    ...
    for part_idx in range(len(parts) - 1):
        if parts[part_idx : part_idx + len(marker)] == marker:
            relocated_path = REPOSITORY_ROOT.joinpath(*parts[part_idx:]).resolve()
...
def load_image_source_info(
    ...
    with RmbData(str(rmb_path)) as rmb_data:
        ...
        feature_attrs = rmb_data[image_feature_key].attrs if has_stored_features else {}
        ...
        if vae_checkpoint_override is not None:
            ...
            vae_checkpoint = resolve_recorded_path(feature_attrs["model"])
        ...
        if camera_name_override is not None:
            ...
            camera_name = str(rmb_data.attrs["online_pb_image_vae_camera_name"])
            ...
            camera_name = infer_camera_name(image_feature_key, latent_dim)
...
def get_prediction_indexes(
    ...
    assert len(start_idxes) > 0, (num_raw_steps, data_info)
    ...
    return start_idxes, latest_observation_idxes, target_idxes * skip
...
def read_video(
            ...
            if encode_features:
                ...
                if len(encode_batch) == BATCH_SIZE:
                    encoded_batches.append(
                        encode_image_batch(vae, encode_batch, image_size, device)
                    )
                    encode_batch = []
            frame_idx += 1
    ...
    encoded_features = np.concatenate(encoded_batches) if encode_features else None
...
def load_prediction_sequences(
    ...
    with RmbData(str(rmb_path)) as rmb_data:
        ...
        image_feature = convert_data_to_policy(
            get_skipped_data_seq(image_feature, image_feature_key, skip),
            image_feature_key,
        )
        ...
        if state_keys:
            ...
            state = np.zeros((len(image_feature), 0), dtype=np.float32)
    ...
    sequences = {
        "state": normalize_data(state, model_meta_info["state"]),
        "action": normalize_data(action, model_meta_info["action"]),
        "image_feature": normalize_data(
            image_feature,
            model_meta_info["image_feature"],
        ),
    }
    return sequences, online_pb.astype(np.float32), raw_time
...
def predict_farthest_features(
    ...
    for batch_start in range(0, len(start_idxes), BATCH_SIZE):
        batch_start_idxes = start_idxes[batch_start : batch_start + BATCH_SIZE]
        batch_observation_idxes = latest_observation_idxes[
            batch_start : batch_start + BATCH_SIZE
        ]
        time_idxes = np.clip(
            batch_start_idxes[:, np.newaxis] + np.arange(horizon),
            0,
            episode_len - 1,
        )
        batch = {
            key: torch.tensor(value[time_idxes], dtype=torch.float32, device=device)
        ...
        material_pbs = np.concatenate(
            [
                ...
                np.broadcast_to(
                    ...
                    (len(batch_start_idxes), *reference_pbs.shape),
                ),
            ],
            axis=1,
        )
        ...
        repeated_batch = {
            key: value.repeat_interleave(num_conditions, dim=0)
            for key, value in batch.items()
        }
        material_pbs_tensor = torch.tensor(
            material_pbs.reshape(-1, material_pbs.shape[-1]),
            dtype=torch.float32,
            device=device,
        )
        with torch.inference_mode():
            prediction = policy(repeated_batch, material_pbs_tensor)["image_feature"]
        farthest_prediction = prediction[:, -1].reshape(
            len(batch_start_idxes),
            num_conditions,
            -1,
        )
        predicted_batches.append(farthest_prediction.cpu().numpy())
    ...
    return predicted_features, np.concatenate(online_pb_batches)
...
def decode_image_features(
    ...
    reconstructed = np.round(255.0 * reconstructed).clip(0, 255).astype(np.uint8)
    return reconstructed.reshape(*features.shape[:-1], *reconstructed.shape[1:])
...
def get_fitted_image_geometry(
    ...
    return resized_width, resized_height, x, y


def fit_image(image: np.ndarray, width: int, height: int) -> np.ndarray:
    resized_width, resized_height, x, y = get_fitted_image_geometry(
        image,
        width,
        height,
    )
    resized = cv2.resize(
        ...
        interpolation=(
            cv2.INTER_AREA if resized_width < image.shape[1] else cv2.INTER_LINEAR
        ),
    )
...
def add_label(image: np.ndarray, text: str, x: int, y: int) -> None:
    cv2.putText(
        image,
        text,
        (x + 7, y + 20),
...
def format_pb(pb: np.ndarray) -> str:
    return "[" + ", ".join(f"{value:.4f}" for value in pb) + "]"
...
def make_canvas(
    ...
    original_panel = fit_image(original_frame, top_width, top_height)
    online_bgr = cv2.cvtColor(reconstructed_rgb[0], cv2.COLOR_RGB2BGR)
    online_panel = fit_image(online_bgr, output_width - top_width, top_height)
    canvas[:top_height, :top_width] = original_panel
    canvas[:top_height, top_width:] = online_panel
    add_label(canvas, f"Original {camera_name}  t={target_time:.2f}s", 0, 0)
    add_label(
        canvas,
    ...
    bottom_edges = np.rint(np.linspace(0, output_width, 4)).astype(int)
    for panel_idx, object_id in enumerate(REFERENCE_OBJECT_IDS):
        ...
        panel = fit_image(reconstructed_bgr, panel_width, panel_height)
        canvas[top_height:, x0:x1] = panel
        _, _, image_x, image_y = get_fitted_image_geometry(
            reconstructed_bgr,
            panel_width,
            panel_height,
        )
...
def get_output_path(
    ...
    return output_dir / f"{rmb_path.stem}_{camera_name}_online_pb_reconstruction.mp4"


def main() -> None:
    args = parse_args()
    assert args.rmb_path.is_dir(), args.rmb_path
    assert args.output_width > 0 and args.output_width % 2 == 0, args.output_width
    assert args.output_height > 0 and args.output_height % 2 == 0, args.output_height
    rmb_paths = sorted(Path(path) for path in find_rmb_files(str(args.rmb_path)))
    assert rmb_paths, args.rmb_path
    assert all(path.suffix == ".rmb" for path in rmb_paths), rmb_paths
    if len(rmb_paths) > 1:
        assert args.output is None, "Use --output_dir for multiple RMB episodes."
    else:
        assert args.output_dir is None or args.output is None

    checkpoint_paths = [load_wp4_checkpoint_path(path) for path in rmb_paths]
    checkpoint_path = checkpoint_paths[0]
    assert all(path == checkpoint_path for path in checkpoint_paths), checkpoint_paths
    model_meta_info = load_model_meta_info(checkpoint_path)
    assert model_meta_info["data"]["horizon"] == 16, model_meta_info["data"]
    image_feature_key = model_meta_info["data"]["image_feature_key"]
    assert image_feature_key.startswith("image_vae"), (
        "This video requires an ImageVAE-based WP4",
        image_feature_key,
    )
    latent_dim = model_meta_info["policy"]["args"]["image_feature_dim"]
    ...
    vae_checkpoint, camera_name, _ = image_source_info[0]
    assert all(
        info[:2] == (vae_checkpoint, camera_name) for info in image_source_info
    ), image_source_info

    assert torch.cuda.is_available(), "WP4 reconstruction requires a CUDA GPU."
    device = torch.device("cuda")
    policy = load_policy(checkpoint_path, model_meta_info, device)
    reference_pb_table, _ = load_pb_table(checkpoint_path, model_meta_info)
    reference_pbs = reference_pb_table[list(REFERENCE_OBJECT_IDS)]
    vae = AutoModel.load_from_folder(str(vae_checkpoint)).eval().to(device)
    vae.requires_grad_(False)
    assert vae.model_config.latent_dim == latent_dim, (
        vae.model_config.latent_dim,
        latent_dim,
    )
    _, image_height, image_width = vae.model_config.input_dim
    image_size = (image_width, image_height)
    output_size = (args.output_width, args.output_height)

    print(f"WP4 checkpoint: {checkpoint_path}")
    print(f"ImageVAE checkpoint: {vae_checkpoint}")
    print(f"Image feature: {image_feature_key} ({latent_dim}D)")
    ...
    for episode_idx, (rmb_path, source_info) in enumerate(
        zip(rmb_paths, image_source_info, strict=True),
        start=1,
    ):
        _, _, has_stored_features = source_info
        with RmbData(str(rmb_path)) as rmb_data:
            num_raw_steps = rmb_data[DataKey.TIME].shape[0]
        start_idxes, latest_observation_idxes, target_raw_idxes = (
            get_prediction_indexes(num_raw_steps, model_meta_info)
        )
        video_path = rmb_path / f"{camera_name}_rgb_image.rmb.mp4"
        original_frames, encoded_features, source_fps = read_video(
            video_path,
            target_raw_idxes,
            num_raw_steps,
            vae,
        ...
        sequences, online_pb, raw_time = load_prediction_sequences(
            rmb_path,
            model_meta_info,
            encoded_features,
        )
        predicted_features, prediction_online_pbs = predict_farthest_features(
            policy,
            model_meta_info,
            sequences,
            online_pb,
        ...
        reconstructed = decode_image_features(vae, predicted_features, device)
        target_times = raw_time[target_raw_idxes] - raw_time[0]
        ...
        print(
            ...
            f"{len(original_frames)} frames -> {output_path.resolve()}"
        )
