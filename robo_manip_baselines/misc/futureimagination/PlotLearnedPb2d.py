import argparse
from pathlib import Path

import matplotlib

matplotlib.use("Agg")

import matplotlib.pyplot as plt
import numpy as np
import torch


NUM_OBJECTS = 5


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot the five learned two-dimensional PBs in a WP4 checkpoint."
    )
    parser.add_argument(
        "checkpoint",
        type=Path,
        help="WP4 checkpoint containing material_property.weight",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="output PNG path (default: next to the checkpoint)",
    )
    return parser.parse_args()


def load_pb(checkpoint_path: Path) -> np.ndarray:
    checkpoint_path = checkpoint_path.resolve()
    assert checkpoint_path.is_file(), checkpoint_path

    state_dict = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
    material_property = state_dict["material_property.weight"]
    assert material_property.ndim == 2, material_property.shape
    assert material_property.shape[0] >= NUM_OBJECTS, material_property.shape
    assert material_property.shape[1] == 2, material_property.shape

    # The PushT experiment uses object IDs 0 through 4. WP4 may reserve
    # additional, unused embedding rows through its num_objects setting.
    return material_property[:NUM_OBJECTS].detach().numpy()


def get_default_output_path(checkpoint_path: Path) -> Path:
    return checkpoint_path.resolve().with_name(
        f"{checkpoint_path.stem}_learned_pb_2d.png"
    )


def save_plot(pb: np.ndarray, output_path: Path) -> None:
    colors = plt.cm.tab10(np.arange(NUM_OBJECTS))
    x_center = float(pb[:, 0].min() + pb[:, 0].max()) / 2.0
    y_center = float(pb[:, 1].min() + pb[:, 1].max()) / 2.0
    value_range = max(float(np.ptp(pb[:, 0])), float(np.ptp(pb[:, 1])))
    half_range = max(value_range * 0.65, 0.05)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    figure, axis = plt.subplots(figsize=(8, 7))
    axis.axhline(0.0, color="0.65", linewidth=1.0)
    axis.axvline(0.0, color="0.65", linewidth=1.0)

    for object_id, value in enumerate(pb):
        x_offset = -8 if value[0] >= x_center else 8
        y_offset = 8 if value[1] >= y_center else -8
        axis.scatter(
            value[0],
            value[1],
            s=110,
            color=colors[object_id],
            zorder=2,
        )
        axis.annotate(
            f"WrenchPredObject{object_id}\n({value[0]:.4f}, {value[1]:.4f})",
            value,
            xytext=(x_offset, y_offset),
            textcoords="offset points",
            ha="left" if x_offset > 0 else "right",
            va="bottom" if y_offset > 0 else "top",
            color=colors[object_id],
        )

    axis.set_xlim(x_center - half_range, x_center + half_range)
    axis.set_ylim(y_center - half_range, y_center + half_range)
    axis.set_aspect("equal", adjustable="box")
    axis.set_xlabel("PB dimension 1")
    axis.set_ylabel("PB dimension 2")
    axis.set_title("Learned two-dimensional material PBs")
    axis.grid(alpha=0.3)
    figure.tight_layout()
    figure.savefig(output_path, dpi=180)
    plt.close(figure)


def main() -> None:
    args = parse_args()
    pb = load_pb(args.checkpoint)
    output_path = args.output
    if output_path is None:
        output_path = get_default_output_path(args.checkpoint)

    save_plot(pb, output_path)

    for object_id, value in enumerate(pb):
        print(f"WrenchPredObject{object_id}: {value.tolist()}")
    print(output_path.resolve())


if __name__ == "__main__":
    main()
