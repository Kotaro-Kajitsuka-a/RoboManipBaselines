...
class VisualizeHdf5Wrench:
    ...
    def _load_wrench(self):
        with h5py.File(self.hdf5_file, "r") as h5file:
            if "measured_eef_wrench" not in h5file:
                raise KeyError(
                    f"[{self.__class__.__name__}] 'measured_eef_wrench' is not found: {self.hdf5_file}"
                )

            wrench = np.asarray(h5file["measured_eef_wrench"][:], dtype=np.float64)
        ...
        if wrench.ndim != 2:
            raise ValueError(
                f"[{self.__class__.__name__}] measured_eef_wrench must be 2D, got {wrench.shape}"
            )
    ...
    def run(self):
        ...
        fig.suptitle(f"measured_eef_wrench ({self.hdf5_file})", fontsize=TITLE_FONTSIZE)
...
if __name__ == "__main__":
    visualize = VisualizeHdf5Wrench(**vars(parse_argument()))
    visualize.run()
