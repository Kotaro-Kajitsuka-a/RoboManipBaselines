...
# Usage:
# python unstructured/collect_blind_teleop_admittance.py
...
EPISODE_NAME_PATTERN = re.compile(
    r"^(?:.*_)?WrenchPredObject(?P<object_idx>\d+)_world(?P<world_idx>\d+)_"
...
session_dataset_dir = os.path.join(
    dataset_dir,
    "DatasetMujocoXarm7AdmittancePusht",
    "training",
)
os.makedirs(session_dataset_dir, exist_ok=True)

object_dataset_dirs = {}
for object_idx in range(4):
...
def create_trials():
    ...
    trials = []
    for object_idx in range(4):
        env_name = f"MujocoXarm7AdmittancePusht_T{object_idx}"
