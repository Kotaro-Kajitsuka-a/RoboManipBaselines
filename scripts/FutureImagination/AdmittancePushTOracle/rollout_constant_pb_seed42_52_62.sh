...
# Evaluate the constant-PB-trained DP while adapting PB online.
eval_timestamp=$(date +%Y%m%d_%H%M%S)
eval_dir="robo_manip_baselines/dataset/tests/FutureImagination/AdmittancePushTOracle/DP_constant_pb_eval_${eval_timestamp}"
...
for train_seed in 42 52 62; do
  ...
  python robo_manip_baselines/bin/Rollout.py \
    ...
    --demo_name "AdmittancePushTOracleConstant_trainseed${train_seed}_rollseed42_T0" \
    --checkpoint "robo_manip_baselines/checkpoint/DiffusionPolicy/AdmittancePushTOracle/constant_seed${train_seed}/policy_last.ckpt" \
  ...
  python robo_manip_baselines/bin/Rollout.py \
    ...
    --demo_name "AdmittancePushTOracleConstant_trainseed${train_seed}_rollseed42_T1" \
    --checkpoint "robo_manip_baselines/checkpoint/DiffusionPolicy/AdmittancePushTOracle/constant_seed${train_seed}/policy_last.ckpt" \
    --wp4_checkpoint "$wp4_checkpoint" \
    --initial_object_id 0 \
    --online_pb_lr 2.5e-2 \
  ...
  python robo_manip_baselines/bin/Rollout.py \
    ...
    --demo_name "AdmittancePushTOracleConstant_trainseed${train_seed}_rollseed42_T2" \
    --checkpoint "robo_manip_baselines/checkpoint/DiffusionPolicy/AdmittancePushTOracle/constant_seed${train_seed}/policy_last.ckpt" \
    --wp4_checkpoint "$wp4_checkpoint" \
    --initial_object_id 0 \
    --online_pb_lr 2.5e-2 \
  ...
  python robo_manip_baselines/bin/Rollout.py \
    ...
    --demo_name "AdmittancePushTOracleConstant_trainseed${train_seed}_rollseed42_T3" \
    --checkpoint "robo_manip_baselines/checkpoint/DiffusionPolicy/AdmittancePushTOracle/constant_seed${train_seed}/policy_last.ckpt" \
    --wp4_checkpoint "$wp4_checkpoint" \
    --initial_object_id 0 \
    --online_pb_lr 2.5e-2 \
  ...
  for object_id in 0 1 2 3; do
    ...
    find robo_manip_baselines/dataset \
      ...
      -maxdepth 1 \
      -type d \
      -name "RolloutDiffusionPolicyOnlinePb_AdmittancePushTOracleConstant_trainseed${train_seed}_rollseed42_T${object_id}_20*" \
