...
# Run after train_baseline_seed42_52_62.sh.
eval_timestamp=$(date +%Y%m%d_%H%M%S)
eval_dir="robo_manip_baselines/dataset/tests/FutureImagination/DatasetMujocoUR5eLiftingi_LeftScratchVAE9/DP_joint_pos_baseline_eval_${eval_timestamp}"
...
for train_seed in 42 52 62; do
  ...
  for object_id in 0 1 2 4 5 6 7; do
    ...
    find robo_manip_baselines/dataset \
      ...
      -maxdepth 1 \
      -type d \
      -name "RolloutDiffusionPolicy_MujocoUR5eLiftingi_I${object_id}_joint_pos_baseline_trainseed${train_seed}_20*" \
